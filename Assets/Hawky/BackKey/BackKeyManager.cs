using System.Collections.Generic;
using UnityEngine;

namespace Hawky.Backkey
{
    public interface IKeyBack
    {
        public bool OnKeyBack();
    }

    public class BackKeyManager : RuntimeSingleton<BackKeyManager>, IUpdateBehaviour
    {
        public void Update()
        {
            if (Input.GetKeyDown(KeyCode.Escape))
            {
                if (this.keys.Count > 0)
                {
                    var list = new List<IKeyBack>(this.keys);
                    for (int i = list.Count - 1; i >= 0; i--)
                    {
                        if (list[i].OnKeyBack())
                        {
                            return;
                        }
                    }
                }
            }
        }

        private List<IKeyBack> keys = new List<IKeyBack>();

        public void AddListener(IKeyBack key)
        {
#if UNITY_EDITOR
            Debug.Log($"Keyback Listener added: {key.GetType().Name} has been registered.");
#endif
            keys.Add(key);
        }

        public void RemoveListener(IKeyBack key)
        {
#if UNITY_EDITOR
            Debug.Log($"Keyback Listener removed: {key.GetType().Name} has been unregistered.");
#endif
            keys.Remove(key);
        }
    }
}
