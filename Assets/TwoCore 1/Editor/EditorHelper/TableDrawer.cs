...
namespace TwoCore
{
    public class TableDrawer<T>
    {
        ...
        public bool drawCloneButton = false;
        ...
        private bool cloneButtonClick;

        public string SearchText { get; private set; } = "";
        public string ControlLabel { get; set; } = "Controls";
        ...
        public T ClonedElement { get; private set; }
        ...
        private void Init()
        {
            ...
            if (drawCloneButton) columnDrawers.Add(CreateCloneColDrawer());
            if (allowSwap)
            {
        ...
        private ColDrawer<T> CreateCloneColDrawer()
        {
            return new ColDrawer<T>("", 50, (ele, i) =>
            {
                if (ele is BaseData baseData && i < FullList.Count)
                {
                    Draw.BeginDisabledGroup(disableInlineAddButton);
                    if (Draw.Button("Clone", Color.yellow, 50))
                    {
                        cloneButtonClick = true;
                        {
                            ClonedElement = (T)baseData.Clone();
                        }
                    }
        ...
        private void UpdateElementCloned()
        {
            if (cloneButtonClick)
            {
                FullList.Add(ClonedElement);
                onElementAdded?.Invoke(ClonedElement);
        ...
        private void DrawControl()
        {
            ...
            {
                Draw.Space();
                showControl = Draw.FitToggleGroup(showControl, ControlLabel);
                if (showControl)
                {
                    Draw.BeginVertical();
                    {
