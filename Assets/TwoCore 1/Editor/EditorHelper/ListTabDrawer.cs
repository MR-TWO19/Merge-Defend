...
namespace TwoCore
{
    public abstract class ListTabDrawer<T> where T : ICloneable
    {
        ...
        protected virtual Color GetHeaderBackgroundColor(T element) => Color.white;
        ...
        {
            DrawHorizontalHeaderImpl(PagingList.ToList());
        }

        private void DrawHorizontalHeader()
        {
            DrawHorizontalHeaderImpl(fullList.ToList());
        }

        void DrawHorizontalHeaderImpl(List<T> _list)
        {
            do
            {
                float totalW = 0;
                ...
                Draw.BeginHorizontal();
                {
                    while (_list.Count > 0)
                    {
                        ...
                        if (totalW + w <= Screen.width)
                        {
                            ...
                            GUI.backgroundColor = GetHeaderBackgroundColor(ele);
        ...
        private void OnCloneClicked(T origin)
        {
            T newElement = (T)origin.Clone();
            fullList.Add(newElement);
            onElementCloned(newElement, origin);
            onElementAdded(newElement);
            SelectedElement = newElement;
            OnTabChanged(SelectedElement, SelectedIndex);
        }
